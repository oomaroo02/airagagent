...
build_function() {
  ...
  if grep --quiet "built successfully" $TARGET_DIR/fn_build.log; then
     ...
     oci raw-request --region $TF_VAR_region --http-method GET --target-uri "https://${TF_VAR_ocir}/20180419/docker/token" --auth=instance_principal  | jq -r .data.token | docker login -u BEARER_TOKEN --password-stdin ${TF_VAR_ocir}
